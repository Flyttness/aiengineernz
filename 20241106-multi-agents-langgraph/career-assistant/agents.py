...
def job_search_node(state):
    ...
    state["messages"].append(HumanMessage(content=output.get("output"), name="JobSearcher"))
    return state
...
def resume_analyzer_node(state):
    ...
    output = analyzer_agent.invoke({"messages": state["messages"]}, {"callbacks": [state["callback"]], "recursion_limit": 2})
    state["messages"].append(HumanMessage(content=output.get("output"), name="ResumeAnalyzer"))
    return state
...
def cover_letter_generator_node(state):
    ...
    output = generator_agent.invoke({"messages": state["messages"]}, {"callbacks": [state["callback"]]})
...
def web_research_node(state):
    ...
    output = research_agent.invoke({"messages": state["messages"]}, {"callbacks": [state["callback"]]})
    state["messages"].append(HumanMessage(content=output.get("output"), name="WebResearcher"))
    return state
